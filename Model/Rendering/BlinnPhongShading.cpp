...
vec3 BlinnPhongShading::shading(shared_ptr<Scene> scene, HitInfo& info, vec3 lookFrom, vec3 ambientLight, vector<shared_ptr<Light>> lights){
    ...
    for(const shared_ptr<Light>&light: lights){
        ...
        //Atenuación hasta p
        att = light->attenuation(info.p);
        sumatorio += att *(light->getId()*info.mat_ptr->getDiffuse(info.uv) * cosLN + light->getIs()*info.mat_ptr->Ks*pow(cosNH,info.mat_ptr->shininess)) +
                light->getIa()*info.mat_ptr->Ka;
    }
