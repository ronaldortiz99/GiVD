...
void SceneFactoryData::read(const QJsonObject &json)
{
    ...
    if (json.contains("base") && json["base"].isObject()) {
        ...
        if (jbase.contains("type") && jbase["type"].isString()) {
            ...
            o->read(jbase);
            shared_ptr<FittedPlane> plane = std::static_pointer_cast<FittedPlane>(o);
            scene->setBasePlane(plane);
...
shared_ptr<Object> SceneFactoryData::objectMaps(int i, int j) {
    ...
    cout << "1 VxMin: " << mapping->Vxmin << " VzMin: " << mapping->Vzmin << endl;
    cout << "1 VxMax: " << mapping->Vxmax << " VzMax: " << mapping->Vzmax << endl;
    cout << "1 RxMin: " << mapping->Rxmin << " RzMin: " << mapping->Rzmin << endl;
    cout << "1 RxMax: " << mapping->Rxmax << " RzMax: " << mapping->Rzmax << endl;
    float x = dades[i].second[j].x;
    float y = dades[i].second[j].y;
    float z = dades[i].second[j].z;
    float px = (mapping->Vxmax - mapping->Vxmin)/(mapping->Rxmax - mapping->Rxmin);
    float vx = px * (x - mapping->Rxmin) + mapping->Vxmin;
    float vy = (z - mapping->attributeMapping[i]->minValue)  / (mapping->attributeMapping[i]->maxValue - mapping->attributeMapping[i]->minValue);
    float pz = (mapping->Vzmax - mapping->Vzmin)/(mapping->Rzmax - mapping->Rzmin);
    float vz = pz * (y - mapping->Rzmin) + mapping->Vzmin;
    cout << "2 VxMin: " << mapping->Vxmin << " VzMin: " << mapping->Vzmin << endl;
    cout << "2 VxMax: " << mapping->Vxmax << " VzMax: " << mapping->Vzmax << endl;
    cout << "2 RxMin: " << mapping->Rxmin << " RzMin: " << mapping->Rzmin << endl;
    cout << "2 RxMax: " << mapping->Rxmax << " RzMax: " << mapping->Rzmax << endl;
    shared_ptr<TG> t1 = make_shared<ScaleTG>(ScaleTG(vec3(vy)));
    shared_ptr<TG> t2 = make_shared<TranslateTG>(TranslateTG(vec3(vx,mapping->Vymax,vz)));
    o->aplicaTG(t1);
    o->aplicaTG(t2);
