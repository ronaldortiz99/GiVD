...
Lambertian::Lambertian(const vec3& a, const vec3& d, const vec3& s, const float k, const float o, const vec3& t, const float nu):
...
bool Lambertian::scatter(const Ray& r_in, const HitInfo& rec, vec3& color, Ray & r_out) const  {
    vec3 target = rec.normal + Hitable::RandomInSphere();
