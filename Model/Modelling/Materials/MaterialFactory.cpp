...
shared_ptr<Material> MaterialFactory::createMaterial(vec3 a, vec3 d, vec3 s, float beta, float opacity, MATERIAL_TYPES t, vec3 kt, float nu ) {
    shared_ptr<Material> m;
    switch (t) {
    case LAMBERTIAN:
        m = make_shared<Lambertian>(a, d, s, beta, opacity, kt, nu);
        break;
    case METAL:
        m = make_shared<Metal>(a, d, s, beta, opacity, kt, nu);
        break;
    case TRANSPARENT:
        m = make_shared<Transparent>(a, d, s, beta, opacity, kt, nu);
    default:
        break;
    }
    return m;
...
MaterialFactory::MATERIAL_TYPES MaterialFactory::getIndexType(Material *  m) {
    if (dynamic_cast<Lambertian*>(m) != nullptr) {
        return MATERIAL_TYPES::LAMBERTIAN;
    }
    if (dynamic_cast<Metal*>(m) != nullptr) {
        return MATERIAL_TYPES::METAL;
    }
    if (dynamic_cast<MaterialTextura*>(m) != nullptr) {
        return MATERIAL_TYPES::MATERIALTEXTURA;
    }
    if (dynamic_cast<Transparent*>(m) != nullptr){
        return MATERIAL_TYPES::TRANSPARENT;
    }
    return MATERIAL_TYPES::LAMBERTIAN;
}
