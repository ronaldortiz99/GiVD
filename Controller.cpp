...
bool Controller::createScene(vec3 min, vec3 max) {
    // Create a single Box
    scene = make_shared<Scene>();
    auto box = make_shared<Box>(max, min, 1.0);
...
bool Controller::createScene(const QString &fileName){
    scene = make_shared<Scene>();
    auto mesh = make_shared<Mesh>(fileName, 1.0);
